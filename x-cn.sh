...
install() {
    bash <(curl -Ls https://cors.ssss.fun/https://cors.ssss.fun/https://raw.githubusercontent.com/womade/x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    ...
    bash <(curl -L -s https://cors.ssss.fun/https://raw.githubusercontent.com/teddysun/across/master/bbr.sh)
    echo ""
    before_show_menu
}

update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/womade/x-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
